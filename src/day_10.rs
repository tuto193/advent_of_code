...
pub fn day_10() {
    ...
    for instruction in input.split("\n").into_iter() {
        ...
        let (found_important, important_cycle_strength) = advance_cycle(&mut current_cycle, times, &mut x, add_x);
        if found_important {
...
fn get_drawn_pixels_from_cycle(cycle: &mut usize, times: usize, x: &mut i32, add_x: i32) -> Vec<usize> {
    let mut which_cycles: Vec<usize> = vec![];
    let pixel_positions: Vec<i32> = ((*x - 1).max(0).min(239)..(*x + 1).max(0).min(240)).collect();
    let pixel_positions: Vec<usize> = pixel_positions.into_iter().map(|int| int.try_into().unwrap() ).collect();
    for _ in 0..times {
        if pixel_positions.contains(&*cycle) {
...
pub fn day_10_part2() {
    ...
    for instruction in input.split("\n").into_iter() {
        ...
        let mut more_cycles_to_draw = get_drawn_pixels_from_cycle(&mut current_cycle, times, &mut x, add_x);
        // signal_strengths.push(important_cycle_strength);
