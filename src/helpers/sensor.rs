...
impl Sensor {
    ...
    fn is_within_row(&self, row: isize) -> bool {
        let top = if self.radius as isize > self.centre.1 {
            0
        } else {
            self.centre.1 - self.radius as isize
        };
    ...
    pub fn get_all_around_radius(&self) -> Vec<Position> {
       let mut around_radius = vec![];
        let large_lim: isize = 4000000;
        let left = self.centre.0 - self.radius as isize - 1;
        let left = left.max(0).min(large_lim);
        let right = self.centre.0 + self.radius as isize + 1;
        let right = right.max(0).min(large_lim);
        let top = self.centre.1 - self.radius as isize - 1;
        let top = top.max(0).min(large_lim);
        let bottom = self.centre.1 + self.radius as isize + 1;
        let bottom = bottom.max(0).min(large_lim);
